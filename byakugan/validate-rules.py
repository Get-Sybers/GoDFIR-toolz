"""The build gate for the baked Elastic detection rules-as-code (rules/).

The rule files are DATA — one YAML per detection, ES|QL or EQL, each carrying
the contract for the TAGGED EVIDENCE LINE it produces — baked into the
get-sybers/byakugan image at ``/rules`` (the deployment's rules-as-code the
engine's ``stix-export`` resolves indicator patterns from; a runtime mount
overrides the baked set). This script is their validator, run against the
baked tree during the image build so a malformed or silently-dropped rule
FAILS THE BUILD; it validates the same way standalone
(``python3 byakugan/validate-rules.py``). It moved here with the rules from
DX_DFIR's retired host-python package, rigor intact.

PINNED. ``PINNED_IDS`` enumerates the detection set: a rule can neither be
dropped silently nor added unnoticed — changing the set means changing the
pin here, tracked and reviewed like the rules themselves. The one
``threat_match`` rule (``rules/cti/``) validates standalone; the cti-* index
template its ``threat_index`` reads lives in the Byakugan engine
(``byakugan.exchange.cti``), which carries the deep template cross-checks.
...
Rule file shape (``rules/README.md`` documents the model in full)::
    ...
    language: esql | eql
    index: [logs-dxdfir.evtx-*]         # the data streams the query reads
...
# The pinned detection set (rules/README.md "Coverage"): the top-level rule
# ids, exactly — main() fails the build on any drift. The cti/ threat_match
# rule sits beside the set, not in it.
PINNED_IDS = (
...
def load_cti_contract(rule_path: str = CTI_RULE,
                      template_path: str | None = None) -> tuple[dict, dict | None]:
    """The indicator-match rule (rules/cti/) as data — ``(rule, template)``,
    with ``template`` None unless a cti-* index template path is handed in (a
    copy a deployment kept; the canonical one lives in the Byakugan engine).
    The rule gets ``_path`` and a defaulted ``risk_score`` exactly as
    :func:`load` gives the rule set."""
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        prog="validate-rules.py",
        description="Validate the baked Elastic detection rules-as-code set "
                    "(the image build gate); print a JSON summary.")
    ...
    try:
        ...
        got = sorted(r["id"] for r in rules)
        if got != sorted(PINNED_IDS):
            raise ValueError(
                "the detection set is pinned: expected exactly "
                f"{sorted(PINNED_IDS)}, got {got} — a dropped or added rule "
                "changes PINNED_IDS here, reviewed like the rule itself")
        ...
        # detection; it validates standalone (its cti-* template is engine-side).
        if os.path.isfile(cti_rule):
            validate_indicator_match(*load_cti_contract(cti_rule))
