...
The deployment's detections for **Elastic's own Detection Engine** as data:
one YAML file per rule, ES|QL or EQL, each carrying the contract for the
**tagged evidence line** it produces. Baked into the `get-sybers/byakugan`
image at `/rules` (the engine's `stix-export` resolves indicator patterns
from them; a runtime mount overrides the baked set), validated and pinned at
image build by [`../validate-rules.py`](../validate-rules.py). They moved
here from DX_DFIR's retired host-python package — detections outside the
Byakugan engine live in this repository, baked at build time, like the
signatures image's rulesets. Their ancestry is the retired Kusto registry,
re-expressed; each rule's `source` block keeps that provenance.

> **The set is pinned.** `validate-rules.py` carries `PINNED_IDS`: a
> detection can neither be dropped silently nor added unnoticed — the image
> build fails on any drift, and changing the set means changing the pin,
> reviewed like the rules themselves.

```bash
python3 byakugan/validate-rules.py    # JSON summary, exit 1 on a bad rule — the same gate the image build runs
```
...
| `created` | no (STIX export: yes) | ISO 8601 date the rule was authored (`"2026-09-02"`). `byakugan stix-export` makes it the indicator's `created` / `valid_from` — STIX 2.1 §3.2: `created` never changes, so it cannot be the export clock; a rule without it is skipped and counted |
...
| `language` | yes | `esql` or `eql` — also the STIX indicator's `pattern_type` (a trust-group value beyond `pattern-type-ov`, see the engine's [docs/STIX-Exchange.md](https://github.com/Get-Sybers/byakugan/blob/main/docs/STIX-Exchange.md)) |
...
`validate-rules.py`'s `validate()` fails fast: the first problem raises
...
Rules read the Byakugan data streams: **`logs-dxdfir.<type>-<namespace>`** for
...
(`logs-dxdfir.*`) resolves the guid through its declared `car_join.provenance`
...
Detection Engine **`threat_match`** rule: it compares `logs-dxdfir.*` /
...
of OpenCTI's STIX indicators that `byakugan cti-pull` writes (the cti-*
index template and the pattern mapping deciding which STIX comparison lands
where live in the engine,
[`byakugan/exchange/cti/`](https://github.com/Get-Sybers/byakugan/tree/main/byakugan/exchange/cti)). Same
...
`matched.{field,atomic,id,index}`), which `byakugan cti-sightings` turns
into STIX sightings of the platform's own indicator.

It is Byakugan-native, not a registry port, so it lives beside the pinned
top-level rule set rather than in it. `validate-rules.py` validates it
standalone at image build; the deep cross-checks — every `threat_mapping`
value a field the copy fills, every `threat_index` pattern one the template
covers — belong to the engine, where the cti-* template and pattern mapping
live (`validate_indicator_match()` still runs them when handed a template
copy).

## Coverage: KQL -> ES|QL / EQL

Every detection the retired registry carried, ported or stub (the image
build fails if one goes missing):

| registry id | Kusto source | Elastic | reads | status | note |
|---|---|---|---|---|---|
| `win-eventlog-cleared` | `host.EvtxEcmdJson` | EQL | `logs-dxdfir.evtx-*` | **ported** | `any where` over `event.code` / `winlog.channel` (`:` = the KQL `=~`); engine-stamped |
| `win-defender-tamper` | `host.EvtxEcmdJson` | ES\|QL line | `logs-dxdfir.evtx-*` | **ported** | `WHERE ... IN`, inline `threat.*` via `EVAL` |
| `win-service-suspicious-path` | `host.EvtxEcmdJson` | ES\|QL line | `logs-dxdfir.evtx-*` | **ported** | `COALESCE(winlog.event_data.ImagePath, ServiceFileName)`; KQL `(?i)` regex -> `TO_LOWER` + Lucene `RLIKE` (no `\b`: end-or-non-alnum instead) |
...
- **Ingest pipelines** (native -> ECS) for the `logs-dxdfir.*` streams — the
