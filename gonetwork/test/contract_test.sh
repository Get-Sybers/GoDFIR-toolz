#!/usr/bin/env bash
# gonetwork contract smoke (docs/framework 02 §2.3, 06 §6.2): runs the tool in
...
run() { # run [extra env...] -> stdout captured, exit code in $code
    ...
    if [[ "${CONTRACT_LOCAL:-0}" == "1" ]]; then
        env "${envs[@]}" "$work/gonetwork" >"$work/stdout" 2>"$work/stderr" || code=$?
    else
        local docker_env=()
        ...
        docker run --rm --network none --read-only --cap-drop ALL \
            ...
            get-sybers/gonetwork:latest >"$work/stdout" 2>"$work/stderr" || code=$?
    fi
    return $code
}

if [[ "${CONTRACT_LOCAL:-0}" == "1" ]]; then
    (cd "$tool_dir" && CGO_ENABLED=0 go build -o "$work/gonetwork" .)
    export GONETWORK_INPUT_DIR="$in" GONETWORK_OUT_DIR="$out" GONETWORK_WORK_DIR="$scratch"
    envs=("GONETWORK_INPUT_DIR=$in" "GONETWORK_OUT_DIR=$out" "GONETWORK_WORK_DIR=$scratch")
else
    (cd "$repo" && docker build -q -t get-sybers/gonetwork:latest -f gonetwork/Dockerfile .)
    chmod 777 "$out"
    envs=()
fi
...
[[ "$(summary records)" == "6" ]] || fail "records $(summary records)"
ls "$out"/*/gonetwork.jsonl >/dev/null 2>&1 || fail "record file missing"
...
code=0; run "${envs[@]/GONETWORK_INPUT_DIR=$in/GONETWORK_INPUT_DIR=$in/missing}" GONETWORK_INPUT_DIR="$in/missing" || code=$?
[[ $code -eq 2 ]] || fail "config-error exit $code, want 2"

echo "gonetwork contract smoke: PASS"
