#!/usr/bin/env bash
# goauditd contract smoke (docs/framework 02 §2.3, 06 §6.2): runs the tool in
...
run() { # run [extra env...] -> stdout captured, exit code in $code
    ...
    if [[ "${CONTRACT_LOCAL:-0}" == "1" ]]; then
        env "${envs[@]}" "$work/goauditd" >"$work/stdout" 2>"$work/stderr" || code=$?
    else
        local docker_env=()
        ...
        docker run --rm --network none --read-only --cap-drop ALL \
            ...
            get-sybers/goauditd:latest >"$work/stdout" 2>"$work/stderr" || code=$?
    fi
    return $code
}

if [[ "${CONTRACT_LOCAL:-0}" == "1" ]]; then
    (cd "$tool_dir" && CGO_ENABLED=0 go build -o "$work/goauditd" .)
    export GOAUDITD_INPUT_DIR="$in" GOAUDITD_OUT_DIR="$out" GOAUDITD_WORK_DIR="$scratch"
    envs=("GOAUDITD_INPUT_DIR=$in" "GOAUDITD_OUT_DIR=$out" "GOAUDITD_WORK_DIR=$scratch")
else
    (cd "$repo" && docker build -q -t get-sybers/goauditd:latest -f goauditd/Dockerfile .)
    chmod 777 "$out"
    envs=()
fi
...
ls "$out"/*/goauditd.jsonl >/dev/null 2>&1 || fail "record file missing"
...
code=0; run "${envs[@]/GOAUDITD_INPUT_DIR=$in/GOAUDITD_INPUT_DIR=$in/missing}" GOAUDITD_INPUT_DIR="$in/missing" || code=$?
[[ $code -eq 2 ]] || fail "config-error exit $code, want 2"

echo "goauditd contract smoke: PASS"
