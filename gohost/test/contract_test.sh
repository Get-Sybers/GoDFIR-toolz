#!/usr/bin/env bash
# gohost contract smoke (docs/framework 02 §2.3, 06 §6.2): runs the tool in
...
run() { # run [extra env...] -> stdout captured, exit code in $code
    ...
    if [[ "${CONTRACT_LOCAL:-0}" == "1" ]]; then
        env "${envs[@]}" "$work/gohost" >"$work/stdout" 2>"$work/stderr" || code=$?
    else
        local docker_env=()
        ...
        docker run --rm --network none --read-only --cap-drop ALL \
            ...
            get-sybers/gohost:latest >"$work/stdout" 2>"$work/stderr" || code=$?
    fi
    return $code
}

if [[ "${CONTRACT_LOCAL:-0}" == "1" ]]; then
    (cd "$tool_dir" && CGO_ENABLED=0 go build -o "$work/gohost" .)
    export GOHOST_INPUT_DIR="$in" GOHOST_OUT_DIR="$out" GOHOST_WORK_DIR="$scratch"
    envs=("GOHOST_INPUT_DIR=$in" "GOHOST_OUT_DIR=$out" "GOHOST_WORK_DIR=$scratch")
else
    (cd "$repo" && docker build -q -t get-sybers/gohost:latest -f gohost/Dockerfile .)
    chmod 777 "$out"
    envs=()
fi
...
ls "$out"/*/gohost.jsonl >/dev/null 2>&1 || fail "record file missing"

# 2. rerun converges: everything skipped, still exit 0
code=0; run "${envs[@]}" || code=$?
[[ $code -eq 0 && "$(summary skipped)" == "4" ]] || fail "rerun not idempotent"

# 3. config error: missing input dir -> exit 2
code=0; run "${envs[@]/GOHOST_INPUT_DIR=$in/GOHOST_INPUT_DIR=$in/missing}" GOHOST_INPUT_DIR="$in/missing" || code=$?
[[ $code -eq 2 ]] || fail "config-error exit $code, want 2"

echo "gohost contract smoke: PASS"
