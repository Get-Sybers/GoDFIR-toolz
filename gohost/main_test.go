package main

// The thin binary's wiring: the embedded host.Tool drives the shared batch
// runtime under this tool's contract (the substantive parser tests live in
// host/).

import (
	"bytes"
	"strings"
	"testing"

	"github.com/Get-Sybers/GoDFIR-toolz/gohost/host"
	"github.com/Get-Sybers/GoDFIR-toolz/pinfo/batch"
)

func TestBinaryBatchContract(t *testing.T) {
	env := map[string]string{
		"GOHOST_INPUT_DIR": t.TempDir(), "GOHOST_OUT_DIR": t.TempDir(),
		"GOHOST_WORK_DIR": t.TempDir(),
	}
	var out bytes.Buffer
	code := batch.Run(host.Tool, batch.Options{Version: "test", Contract: contractYML},
		func(k string) string { return env[k] }, &out)
	if code != 1 || !strings.Contains(out.String(), `"status":"nothing"`) {
		t.Fatalf("empty-tree contract: exit %d, %s", code, out.String())
	}
	if host.Tool.Name != "gohost" {
		t.Fatalf("tool name %q", host.Tool.Name)
	}
}
