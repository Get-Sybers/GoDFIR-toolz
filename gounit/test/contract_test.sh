#!/usr/bin/env bash
# gounit contract smoke (docs/framework 02 §2.3, 06 §6.2): runs the tool in
...
run() { # run [extra env...] -> stdout captured, exit code in $code
    ...
    if [[ "${CONTRACT_LOCAL:-0}" == "1" ]]; then
        env "${envs[@]}" "$work/gounit" >"$work/stdout" 2>"$work/stderr" || code=$?
    else
        local docker_env=()
        ...
        docker run --rm --network none --read-only --cap-drop ALL \
            ...
            get-sybers/gounit:latest >"$work/stdout" 2>"$work/stderr" || code=$?
    fi
    return $code
}

if [[ "${CONTRACT_LOCAL:-0}" == "1" ]]; then
    (cd "$tool_dir" && CGO_ENABLED=0 go build -o "$work/gounit" .)
    export GOUNIT_INPUT_DIR="$in" GOUNIT_OUT_DIR="$out" GOUNIT_WORK_DIR="$scratch"
    envs=("GOUNIT_INPUT_DIR=$in" "GOUNIT_OUT_DIR=$out" "GOUNIT_WORK_DIR=$scratch")
else
    (cd "$repo" && docker build -q -t get-sybers/gounit:latest -f gounit/Dockerfile .)
    chmod 777 "$out"
    envs=()
fi
...
[[ "$(summary records)" == "2" ]] || fail "records $(summary records)"
ls "$out"/*/gounit.jsonl >/dev/null 2>&1 || fail "record file missing"
...
code=0; run "${envs[@]/GOUNIT_INPUT_DIR=$in/GOUNIT_INPUT_DIR=$in/missing}" GOUNIT_INPUT_DIR="$in/missing" || code=$?
[[ $code -eq 2 ]] || fail "config-error exit $code, want 2"

echo "gounit contract smoke: PASS"
