#!/usr/bin/env bash
# goctl contract smoke (docs/framework 02 §2.3, 06 §6.2): runs the tool in
...
run() { # run [extra env...] -> stdout captured, exit code in $code
    ...
    if [[ "${CONTRACT_LOCAL:-0}" == "1" ]]; then
        env "${envs[@]}" "$work/goctl" >"$work/stdout" 2>"$work/stderr" || code=$?
    else
        local docker_env=()
        ...
        docker run --rm --network none --read-only --cap-drop ALL \
            ...
            get-sybers/goctl:latest >"$work/stdout" 2>"$work/stderr" || code=$?
    fi
    return $code
}

if [[ "${CONTRACT_LOCAL:-0}" == "1" ]]; then
    (cd "$tool_dir" && CGO_ENABLED=0 go build -o "$work/goctl" .)
    export GOCTL_INPUT_DIR="$in" GOCTL_OUT_DIR="$out" GOCTL_WORK_DIR="$scratch"
    envs=("GOCTL_INPUT_DIR=$in" "GOCTL_OUT_DIR=$out" "GOCTL_WORK_DIR=$scratch")
else
    (cd "$repo" && docker build -q -t get-sybers/goctl:latest -f goctl/Dockerfile .)
    chmod 777 "$out"
    envs=()
fi
...
ls "$out"/*/goctl.jsonl >/dev/null 2>&1 || fail "record file missing"
...
code=0; run "${envs[@]/GOCTL_INPUT_DIR=$in/GOCTL_INPUT_DIR=$in/missing}" GOCTL_INPUT_DIR="$in/missing" || code=$?
[[ $code -eq 2 ]] || fail "config-error exit $code, want 2"

echo "goctl contract smoke: PASS"
