#!/usr/bin/env bash
# gosyslog contract smoke (docs/framework 02 §2.3, 06 §6.2): runs the tool in
...
run() { # run [extra env...] -> stdout captured, exit code in $code
    ...
    if [[ "${CONTRACT_LOCAL:-0}" == "1" ]]; then
        env "${envs[@]}" "$work/gosyslog" >"$work/stdout" 2>"$work/stderr" || code=$?
    else
        local docker_env=()
        ...
        docker run --rm --network none --read-only --cap-drop ALL \
            ...
            get-sybers/gosyslog:latest >"$work/stdout" 2>"$work/stderr" || code=$?
    fi
    return $code
}

if [[ "${CONTRACT_LOCAL:-0}" == "1" ]]; then
    (cd "$tool_dir" && CGO_ENABLED=0 go build -o "$work/gosyslog" .)
    export GOSYSLOG_INPUT_DIR="$in" GOSYSLOG_OUT_DIR="$out" GOSYSLOG_WORK_DIR="$scratch"
    envs=("GOSYSLOG_INPUT_DIR=$in" "GOSYSLOG_OUT_DIR=$out" "GOSYSLOG_WORK_DIR=$scratch")
else
    (cd "$repo" && docker build -q -t get-sybers/gosyslog:latest -f gosyslog/Dockerfile .)
    chmod 777 "$out"
    envs=()
fi
...
[[ "$(summary records)" == "5" ]] || fail "records $(summary records)"
ls "$out"/*/gosyslog.jsonl >/dev/null 2>&1 || fail "record file missing"

# 2. rerun converges: everything skipped, still exit 0
code=0; run "${envs[@]}" || code=$?
[[ $code -eq 0 && "$(summary skipped)" == "2" ]] || fail "rerun not idempotent"

# 3. config error: missing input dir -> exit 2
code=0; run "${envs[@]/GOSYSLOG_INPUT_DIR=$in/GOSYSLOG_INPUT_DIR=$in/missing}" GOSYSLOG_INPUT_DIR="$in/missing" || code=$?
[[ $code -eq 2 ]] || fail "config-error exit $code, want 2"

echo "gosyslog contract smoke: PASS"
