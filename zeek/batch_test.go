...
// so every Tier-1 tool proves the same contract: env parsing, discovery, one
// output folder per item, idempotency under FORCE, the single summary line and
// the uniform exit codes. It is byte-identical across the tool directories.
...
func fakeBatchTool() batchTool {
	return batchTool{
		...
		process: func(cfg *batchConfig, item, itemDir string, w io.Writer) (int, error) {
...
func runFake(t *testing.T, env map[string]string) (int, batchSummary, string) {
	t.Helper()
	var out bytes.Buffer
	code := runBatch(fakeBatchTool(), envOf(env), &out)
	raw := out.String()
...
func TestBatchPrefix(t *testing.T) {
	if got := batchPrefix("godfir-tool"); got != "GODFIR_TOOL" {
		t.Errorf("batchPrefix = %q", got)
	}
}

func TestBatchItemName(t *testing.T) {
	...
	for item, want := range cases {
		if got := batchItemName("/in", item); got != want {
			t.Errorf("batchItemName(%q) = %q, want %q", item, got, want)
		}
	}
	// an item outside the root falls back to its base name
	if got := batchItemName("/in", "/elsewhere/x"); got != "x" {
		t.Errorf("outside-root item = %q", got)
	}
	long := "/in/" + strings.Repeat("n", 300)
	if got := batchItemName("/in", long); len(got) > 200 {
...
func TestRunBatchPartialAndNothing(t *testing.T) {
...
func TestRunBatchConfigErrors(t *testing.T) {
	in, out := t.TempDir(), t.TempDir()
...
func TestRunBatchSubtoolPrefix(t *testing.T) {
	in, out := t.TempDir(), t.TempDir()
	os.WriteFile(filepath.Join(in, "f"), []byte("x"), 0o644)
	tool := fakeBatchTool()
	tool.name, tool.subtool, tool.prefix = "multi", "sub", "MULTI_SUB"
	var buf bytes.Buffer
	code := runBatch(tool, envOf(map[string]string{"MULTI_SUB_INPUT_DIR": in, "MULTI_SUB_OUT_DIR": out}), &buf)
	var sum batchSummary
...
func TestContractEmbedded(t *testing.T) {
	if !strings.Contains(contractYML, "tool:") || !strings.Contains(contractYML, "entrypoint:") {
