#!/usr/bin/env bash
# gowtmp contract smoke (docs/framework 02 §2.3, 06 §6.2): runs the tool in
...
run() { # run [extra env...] -> stdout captured, exit code in $code
    ...
    if [[ "${CONTRACT_LOCAL:-0}" == "1" ]]; then
        env "${envs[@]}" "$work/gowtmp" >"$work/stdout" 2>"$work/stderr" || code=$?
    else
        local docker_env=()
        ...
        docker run --rm --network none --read-only --cap-drop ALL \
            ...
            get-sybers/gowtmp:latest >"$work/stdout" 2>"$work/stderr" || code=$?
    fi
    return $code
}

if [[ "${CONTRACT_LOCAL:-0}" == "1" ]]; then
    (cd "$tool_dir" && CGO_ENABLED=0 go build -o "$work/gowtmp" .)
    export GOWTMP_INPUT_DIR="$in" GOWTMP_OUT_DIR="$out" GOWTMP_WORK_DIR="$scratch"
    envs=("GOWTMP_INPUT_DIR=$in" "GOWTMP_OUT_DIR=$out" "GOWTMP_WORK_DIR=$scratch")
else
    (cd "$repo" && docker build -q -t get-sybers/gowtmp:latest -f gowtmp/Dockerfile .)
    chmod 777 "$out"
    envs=()
fi
...
[[ -s "$out/var_log_wtmp/gowtmp.jsonl" ]] || fail "record file missing"
...
code=0; run "${envs[@]/GOWTMP_INPUT_DIR=$in/GOWTMP_INPUT_DIR=$in/missing}" GOWTMP_INPUT_DIR="$in/missing" || code=$?
[[ $code -eq 2 ]] || fail "config-error exit $code, want 2"

echo "gowtmp contract smoke: PASS"
